...
export default function OrderDetailScreen() {
  ...
  const handleConfirmAccept = (pickupEtaMin: number) => {
    ...
    accept.mutate(
      ...
      {
        /**
         * ПОСЛЕ ПРИНЯТИЯ — СРАЗУ К ЗАКАЗУ, а не назад в список.
         *
         * `router.back()` возвращал водителя туда, откуда он пришёл, — на
         * список свободных заказов. То есть человек только что взял заказ,
         * и первое, что видит, — чужие заказы, которые ему уже не нужны;
         * до адреса подачи, маршрута и кнопки «Я на месте» надо было идти
         * самому. На это пожаловался владелец 09.09.2026.
         *
         * `replace`, а не `push`: карточка заказа открыта модально, и
         * замена закрывает её заодно — иначе она осталась бы в стеке и
         * «назад» с экрана поездки возвращал бы к описанию уже взятого
         * заказа. То же решение, что при принятии со списка
  ...
  return (
    <Screen edges={['bottom']}>
      ...
      <IncomingOrderModal
        ...
        etaDistanceKm={etaQuery.data?.distanceKm}
