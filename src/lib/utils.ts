...
export function shortenStreetType(address: string): string {
  return address
    ...
    .map((segment) => {
      ...
      /**
       * СОКРАЩАЕМ ПОСЛЕДНЕЕ СЛОВО-ТИП, А НЕ ПЕРВОЕ.
       *
       * «Набережная» бывает и типом («набережная Обводного канала»), и
       * собственным именем улицы («Набережная улица», «Набережная, д. 76»).
       * То же у Проезда, Площади, Бульвара, Линии, Аллеи.
       *
       * Первая версия сокращала первое слово вслепую и съедала название:
       * «наб., д. 76». Вторая требовала слова после него — и на «Набережная
       * улица, 76» выдала «наб. улица, 76»: следующее слово оказалось не
       * названием, а ВТОРЫМ типом (замечено на экране водителя 10.09.2026).
       *
       * Правило, которое держит все три случая: в русском адресе тип стоит
       * либо перед названием («улица Мира»), либо после него («Набережная
       * улица»). Значит, если типов в сегменте два — первый из них имя, а
       * сокращать надо второй. И сокращать вообще нечего, если в сегменте
       * нет ни одного слова, кроме самого типа: от «Набережная, д. 76»
       * осталось бы «наб., д. 76».
       */
      const typeIndexes = tokens
        .map((token, index) => (STREET_TYPE_SHORT[token.toLowerCase()] ? index : -1))
        .filter((index) => index !== -1);
      if (typeIndexes.length === 0) return segment;

      const target = typeIndexes[typeIndexes.length - 1]!;

      // Сокращаем, только если после этого в сегменте останется слово с
      // буквами: иначе от улицы не останется ничего («наб., д. 76»).
      // Считаем ОСТАЛЬНЫЕ слова, а не «не-типы»: в «Набережная улица» имя —
      // это первый из двух типов. Цифра именем не считается: «Набережная
      // 76» — это дом.
      const hasName = tokens.some(
        (token, index) => index !== target && /[А-Яа-яЁёA-Za-z]/.test(token),
      );
      if (!hasName) return segment;

      const short = STREET_TYPE_SHORT[tokens[target]!.toLowerCase()]!;

      return indent + tokens.map((token, index) => (index === target ? short : token)).join(' ');
    })
    .join(',');
}
