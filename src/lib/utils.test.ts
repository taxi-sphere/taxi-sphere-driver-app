...
describe('shortenStreetType — длинный адрес в две строки заголовка', () => {
  ...
  ])('тип улицы перед названием сокращается: %s', (input, expected) => {
  ...
  it.each([
    ...
    'Площадь, 1',
    'Бульвар, д. 9',
  ])('слово-ИМЯ не трогаем: %s', (input) => {
    // «Набережная» бывает и типом, и названием улицы. Слепое сокращение
    // съедало название целиком — оставалось «наб., д. 76».
    expect(shortenStreetType(input)).toBe(input);
  });
  ...
  it('подъезд после дома сохраняется', () => {
    expect(shortenStreetType('улица Ленина, д. 1 подъезд 2')).toBe(
      'ул. Ленина, д. 1 подъезд 2',
    );
  });
});
...
describe('shortenStreetType — тип после названия (1.5.56)', () => {
  it.each([
    ['Набережная улица, 76', 'Набережная ул., 76'],
    ['Береговая улица, 41', 'Береговая ул., 41'],
    ['Мирный переулок, 4', 'Мирный пер., 4'],
    ['Советская улица, 1', 'Советская ул., 1'],
  ])('сокращается ТИП, а не название: %s', (input, expected) => {
    // На экране водителя было «наб. улица, 76»: правило сокращало первое
    // слово, увидев за ним другое слово, — но там стоял второй ТИП, а не
    // название улицы.
    expect(shortenStreetType(input)).toBe(expected);
  });

  it('тип перед названием по-прежнему сокращается', () => {
    expect(shortenStreetType('набережная Обводного канала, 5')).toBe(
      'наб. Обводного канала, 5',
    );
  });

  it('«Набережная улица» без дома тоже читается', () => {
    expect(shortenStreetType('Набережная улица')).toBe('Набережная ул.');
  });

  it('название из цифры и слова не путается с домом', () => {
    expect(shortenStreetType('улица 8 Марта, 12')).toBe('ул. 8 Марта, 12');
  });
});
