...
export function useActiveOrders() {
  ...
  const query = useQuery({
    ...
    gcTime: 60_000,
    /**
     * В поездке опрашиваем чаще: на экране идёт счётчик, и сумму на нём
     * считает сервер.
     *
     * ЗАЧЕМ РАЗНЫЕ ИНТЕРВАЛЫ. Тридцать секунд — правильная частота для
     * карточки заказа, которая почти не меняется. Но с 1.5.46 на ней стоит
     * счётчик, а замерший на полминуты таксометр выглядит сломанным: клиент
     * спрашивает сумму, водитель смотрит на неподвижное число. Десять секунд
     * — та частота, на которой число живёт, а запросов всё ещё втрое меньше,
     * чем при секундном опросе.
     *
     * Вне поездки прежние тридцать: там обновлять нечего.
     */
    refetchInterval: (query) =>
      query.state.data?.some((o) => o.status === 'in_progress') ? 10_000 : 30_000,
