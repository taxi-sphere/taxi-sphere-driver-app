...
export interface EtaEstimateResponse {
  ...
  /**
   * Кнопки быстрого выбора под эту рекомендацию (сервер v1.100.11).
   *
   * Ближнюю лестницу задаёт админ в настройках службы, и телефон о ней
   * иначе не узнает. Поле необязательное: старый сервер его не пришлёт, и
   * тогда набор считает `pickupEtaPresets` в приложении.
   */
  presets?: number[];
}
