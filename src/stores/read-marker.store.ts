  ...
  return create<ReadMarkerState>()(
    persist(
      ...
      }),
      {
        ...
        /**
         * Отметка ставится при ПЕРВОМ запуске, а не при первом открытии
         * раздела (1.5.61).
         *
         * Без отметки сервер счётчик не считает, и до 1.5.61 отметка
         * появлялась только когда водитель сам заходил в раздел. Кто в чат
         * не заходил ни разу, не видел бейджа никогда — сообщения
         * диспетчера копились молча. Теперь считается всё, что пришло после
         * установки или обновления; старая переписка по-прежнему не
         * подсвечивается.
         */
        onRehydrateStorage: () => (state, error) => {
          if (!error && state && !state.lastReadAt) state.markRead();
        },
      },
    ),
  );
